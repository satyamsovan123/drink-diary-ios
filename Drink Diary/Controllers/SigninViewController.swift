...
extension SigninViewController: AuthenticationManagerDelegate {
    func didCompleteAuthentication(_ authenticationManager: AuthenticationManager, authenticationResponse: AuthenticationResponse) {
        DispatchQueue.main.async {
            ...
            if(self.message != Constant.AuthenticationMessage.signinSuccessfulMessage) {
                self.performSegue(withIdentifier: "goToInformationFromSignin", sender: self)
            }
    ...
    func didFailWithError(error: Error) {
        CommonService.logger("Error in sign in - \(error)")
        DispatchQueue.main.async {
            self.stopProcessing()
    ...
    func didReceiveToken(_ authenticationManager: AuthenticationManager, token: String) {
        DispatchQueue.main.async {
            ...
            if(token != "") {
                ...
                self.performSegue(withIdentifier: "goToDrinkDataFromSignin", sender: self)
            }
        }
    }
}
